(function (global, factory) {
  ...
  var johnson;
  var hasRequiredJohnson;

  function requireJohnson () {
  	if (hasRequiredJohnson) return johnson;
  	hasRequiredJohnson = 1;
  	var tarjan = requireScc();

  	johnson = function findCircuits(edges, cb) {
  	    var circuits = []; // Output
  ...
  var johnsonExports = requireJohnson();
  var findCircuits = /*@__PURE__*/getDefaultExportFromCjs(johnsonExports);
  ...
  // fork of https://github.com/d3/d3-sankeyCircular copyright Mike Bostock
    ...
    function sankeyCircular() {
      ...
      function computeNodeBreadths (graph, iterations, id) {
        var columns = Array.from(d3Array.group(graph.nodes, function (d) { return d.column }))
          .sort(function (a, b) { return d3Array.ascending(a[0], b[0]) })
